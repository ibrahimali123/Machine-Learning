...
public class Memory {
	...
	public void initRecall(int type , String clas) throws IOException {
		ImageProcessing img = new ImageProcessing("Training/class" + clas + "/" + type
				+ ".bmp", type);
	...
	public void initVectors(String type, String clas) throws IOException {

		ImageProcessing img1 = new ImageProcessing("Training/class" + clas + "/"
				+ type + ".bmp", Integer.valueOf(type));
	...
	public void recallX(int type) throws NumberFormatException, IOException {
		// x' <---- W * X0
		...
		if(type == 1){
			...
			prevXX = op.MultiplyMatrixWithVector(w, Y0);
		}else{
			prevXX = op.MultiplyMatrixWithVector(w, Z0);
		}
		...
		ImageProcessing img = new ImageProcessing("", type);
		img.writeImage(prevX.vect, type + "result");
	}

	// public void recallXclass3() throws NumberFormatException, IOException {
	// // Y0 * W ----> Y'
	//
	// Matrix op = new Matrix(w.row, w.column);
	...
	// img.writeImage(prevY.vect, "3result");
	// }
	//
	// public void recallXclass8() throws NumberFormatException, IOException {
	// // Z0 * W ----> Z'
	//
	// Matrix op = new Matrix(w.row, w.column);
	...
	// img.writeImage(prevZ.vect, "8result");
	// }
